...
# Função para carregar os pacientes na Treeview
def carregar_pacientes(pacientes_list=None):
    ...
    for paciente in pacientes_to_load:
        tree.insert('', 'end', values=(paciente['id'], paciente['nome'], paciente['data de nascimento'], paciente['genero'],
...
# Função para selecionar um paciente na Treeview
def selecionar_paciente(event):
    ...
    entry_estadocivil.insert(0, values[4])
    entry_profissao.insert(0, values[5])
    entry_telefone.insert(0, str(values[6]))
